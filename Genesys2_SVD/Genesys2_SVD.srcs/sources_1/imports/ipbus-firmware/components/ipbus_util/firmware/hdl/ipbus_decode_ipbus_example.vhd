-- Address decode logic for ipbus fabric
--
-- This file has been AUTOGENERATED from the address table - do not hand edit
--
-- We assume the synthesis tool is clever enough to recognise exclusive conditions
-- in the if statement.
--
-- Dave Newbold, February 2011

library ieee;
use ieee.std_logic_1164.all;
use ieee.numeric_std.all;

package ipbus_decode_ipbus_example is

-- START automatically generated VHDL (Mon May 15 13:44:00 2023)
  constant IPBUS_SEL_WIDTH: positive := 3;
-- END automatically generated VHDL

  subtype ipbus_sel_t is std_logic_vector(IPBUS_SEL_WIDTH - 1 downto 0);
  function ipbus_sel_ipbus_example(addr : in std_logic_vector(31 downto 0)) return ipbus_sel_t;

-- START automatically generated VHDL (Mon May 15 13:44:00 2023)
  constant N_SLV_CSR: integer := 0;
  constant N_SLV_CTRLREG: integer := 1;
  constant N_SLV_RAM: integer := 2;
  constant N_SLV_OUTREG: integer := 3;
  constant N_SLAVES: integer := 4;
-- END automatically generated VHDL

end ipbus_decode_ipbus_example;

package body ipbus_decode_ipbus_example is

  function ipbus_sel_ipbus_example(addr : in std_logic_vector(31 downto 0)) return ipbus_sel_t is
    variable sel: ipbus_sel_t;
  begin

-- START automatically generated VHDL (Mon May 15 13:44:00 2023)
    if    std_match(addr, "--------------00--------------0-") then
      sel := ipbus_sel_t(to_unsigned(N_SLV_CSR, IPBUS_SEL_WIDTH)); -- csr / base 0x00000000 / mask 0x00003002
    elsif std_match(addr, "--------------00--------------1-") then
      sel := ipbus_sel_t(to_unsigned(N_SLV_CTRLREG, IPBUS_SEL_WIDTH)); -- reg / base 0x00000002 / mask 0x00003002
    elsif std_match(addr, "--------------01----------------") then
      sel := ipbus_sel_t(to_unsigned(N_SLV_RAM, IPBUS_SEL_WIDTH)); -- ram / base 0x00010000 / mask 0x00003000
    elsif std_match(addr, "--------------11-------------0--") then
      sel := ipbus_sel_t(to_unsigned(N_SLV_OUTREG, IPBUS_SEL_WIDTH)); -- comp_reg / base 0x00030000 / mask 0x00003002
    else
        sel := ipbus_sel_t(to_unsigned(N_SLAVES, IPBUS_SEL_WIDTH));
    end if;
-- END automatically generated VHDL

    return sel;

  end function ipbus_sel_ipbus_example;

end ipbus_decode_ipbus_example;

